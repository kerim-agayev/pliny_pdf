...
> A privacy-first PDF toolkit. 25 of 32 tools run entirely in your browser — files never
...
> does the opposite: 25 of 32 tools run in your browser via WebAssembly. Open DevTools →
> Network and you'll see zero upload traffic. The few things that genuinely need a server
> (PDF↔Word, OCR, Grayscale, Merge, Edit PDF, PDF↔JPG) are clearly marked, encrypted
> in transit, deleted within 24h. All 32 tools are free — no watermark, no daily cap on local
...
1. **Homepage hero** — headline "32 PDF tools, all free." + tool grid (32 tools).
...
7. **Tools page** — full 32-tool grid with local/cloud badges visible.
...
>
> PlinyPDF runs 25 of its 32 tools entirely in the browser using WebAssembly (pdf-lib +
> pdf.js). Your file never leaves the tab — you can confirm it in DevTools → Network: no
> upload request, no matter the file size. The 7 tools that genuinely need server compute
> (PDF→Word via LibreOffice, Word→PDF, OCR, Grayscale, Merge, Edit PDF, PDF↔JPG) are
...
> a contract or an ID scan, that's a real exposure. PlinyPDF keeps 25 of its 32 tools
...
> Sharing a project that might interest this crowd: PlinyPDF. 25 of 32 PDF ops (merge, split,
